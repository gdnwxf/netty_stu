...
import org.xtwy.oldthrift.codec.MyThriftClientEncode;
...
public class ThriftClient {
		...
		try {
			...
			b.handler(new ChannelInitializer<SocketChannel>() {
				...
				public void initChannel(SocketChannel ch) throws Exception {
					ch.pipeline().addLast(new MyThriftClientEncode());
					ch.pipeline().addLast(
	...
	public static ByteBuf startClient(Object obj) throws Exception {
		ChannelFuture f = b.connect("localhost", 8999).sync();
		f.channel().writeAndFlush(obj);
		f.channel().closeFuture().sync();
		return (ByteBuf) f.channel()
